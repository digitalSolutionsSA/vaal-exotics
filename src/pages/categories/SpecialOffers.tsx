...
import specialBg from "../../assets/new-bg.png";
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      const { data, error } = await supabase
        .from("products")
        .select(
          "id,name,category,price,description,in_stock,stock_count,image_url,images,variants,special,created_at"
        )
        .eq("special", true)
        .order("created_at", { ascending: false });

      if (error) {
        console.error("SpecialOffers fetch error:", error);
        setProducts([]);
        setLoading(false);
        return;
      }
  ...
  return (
    <main className="relative min-h-screen text-black">
      ...
      <div className="relative z-10 mx-auto w-full max-w-[2400px] px-4 sm:px-5 lg:px-6 pt-12 sm:pt-14 pb-20">
        ...
        >
          Hand-picked specials and marked-down products worth grabbing while they last.
        </p>
